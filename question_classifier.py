...
class QuestionClassifier:
    ...
    def __init__(self):
        ...
        # 加载特征词（使用安全读取方法）
        try:
            self.disease_wds = safe_read_file(self.disease_path)
            self.department_wds = safe_read_file(self.department_path)
            self.check_wds = safe_read_file(self.check_path)
            self.drug_wds = safe_read_file(self.drug_path)
            self.food_wds = safe_read_file(self.food_path)
            self.producer_wds = safe_read_file(self.producer_path)
            self.symptom_wds = safe_read_file(self.symptom_path)
            self.deny_words = safe_read_file(self.deny_path)

            # 打印加载的词数量用于调试
            print(f"加载完成 - 疾病词: {len(self.disease_wds)}个, 症状词: {len(self.symptom_wds)}个")
